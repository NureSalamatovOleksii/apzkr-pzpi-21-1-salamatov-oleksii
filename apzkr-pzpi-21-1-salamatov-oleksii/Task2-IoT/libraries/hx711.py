...
class HX711:
    def __init__(self, dout, pd_sck, oled=None):
        self.dout = Pin(dout, Pin.IN)
        self.pd_sck = Pin(pd_sck, Pin.OUT)
        self.offset = 0
        self.scaling_factor = 1
        self.oled = oled  # OLED display
    ...
    def _read_data(self):
        ...
        for _ in range(24):
            ...
            if self.dout.value():
                data |= 1
        # 2's complement conversion for 24-bit data
        if data & 0x800000:
            data -= 1 << 24
        return data
    ...
    def get_units(self, times=3):
        return self.get_value(times) / self.scaling_factor #?
